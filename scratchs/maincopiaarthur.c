...
int menu(save *jogador)
{
    ...
    {             // repetir enquanto
        ...
        {
        case 'N':
            volta = novoJogo(jogador);
            break;
        case 'C':
            volta = carregarJogo(jogador);
            break;
        case 'M':
            mostraCreditos();
            break;
...
int movimentacao(fase *fasea, save *jogador)
{
    ...
    while (caracter != ESC && status != 1 && status != 2) // o usuario pode se movimentar ate clicar ESC, ou o Lolo morrer, ou passsar de fase
    {
        ...
        if (status != 1)
        {
            ...
            {
            case S_CIMA:
                status = contato_lolo(S_CIMA, &pos_lolo, &poder, fasea, jogador);
                break;
            case S_BAIXO:
                status = contato_lolo(S_BAIXO, &pos_lolo, &poder, fasea, jogador);
                break;
            case S_ESQ:
                status = contato_lolo(S_ESQ, &pos_lolo, &poder, fasea, jogador);
                break;
            case S_DIR:
                status = contato_lolo(S_DIR, &pos_lolo, &poder, fasea, jogador);
                break;
            }
        }
...
int movimenta_inimigo(ponto *inimigo, fase *fasea, int *status, int *poder)
{
    ...
    if (fasea->elementos[pos_inimigo.y][pos_inimigo.x] == 'E')
    { // se o caracter na posicao for um E, ou seja, o lolo nao matou ele
        if (inimigo->vivo)
        {
            ...
            // o inimigo so pode se mover para espa�os em branco
